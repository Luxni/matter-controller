/**
 * @license
 * Copyright 2022-2025 Matter Authors
...
export function ClusterServer<const T extends ClusterType, const H extends ClusterServerHandlers<T>>(
    ...
    for (const attributeName in attributeDef) {
        ...
        // logger.info(`check this for REQUIRED Attributes ${Diagnosticon(attributeName)}`)
        if (attributeDef[attributeName].isConditional) {
                ...
                if (conditionMatched && (attributesInitialValues as any)[attributeName] === undefined) {
                    logger.warn(
                        `InitialAttributeValue for "${
                            clusterDef.name
                        }/${attributeName}" is REQUIRED by supportedFeatures: ${Diagnosticon(
                            supportedFeatures,
                        )} but is not set!`,
                    );
                }
            ...
            if (!conditionHasMatched && optionalIf !== undefined && optionalIf.length > 0) {
                ...
                if (conditionMatched && (attributesInitialValues as any)[attributeName] === undefined) {
                    logger.debug(
                        `InitialAttributeValue for "${
                            clusterDef.name
                        }/${attributeName}" is optional by supportedFeatures: ${Diagnosticon(
                            supportedFeatures,
                        )} and is not set!`,
                    );
                }
            ...
            if (!conditionHasMatched && (attributesInitialValues as any)[attributeName] !== undefined) {
                logger.warn(
                    `InitialAttributeValue for "${
                        clusterDef.name
                    }/${attributeName}" is provided but it's neither optional or mandatory for supportedFeatures: ${Diagnosticon(
                        supportedFeatures,
                    )} but is set!`,
                );
            }
        ...
        if (commandDef[name].isConditional) {
            ...
            if (mandatoryIf !== undefined && mandatoryIf.length > 0) {
                ...
                if (conditionMatched && handler === undefined) {
                    logger.warn(
                        `Command "${clusterDef.name}/${name}" is REQUIRED by supportedFeatures: ${Diagnosticon(
                            supportedFeatures,
                        )} but is not set!`,
                    );
                }
            ...
            if (!conditionHasMatched && optionalIf !== undefined && optionalIf.length > 0) {
                ...
                if (conditionMatched && handler === undefined) {
                    logger.debug(
                        `Command "${clusterDef.name}/${name}" is optional by supportedFeatures: ${Diagnosticon(
                            supportedFeatures,
                        )} and is not set!`,
                    );
                }
            ...
            if (!conditionHasMatched && handler !== undefined) {
                logger.warn(
                    `Command "${
                        clusterDef.name
                    }/${name}" is provided but it's neither optional nor mandatory for supportedFeatures: ${Diagnosticon(
                        supportedFeatures,
                    )} but is set!`,
                );
            }
        ...
        if (eventDef[eventName].isConditional) {
            ...
            if (mandatoryIf !== undefined) {
                ...
                if (conditionMatched && (supportedEvents as any)[eventName] === undefined) {
                    logger.warn(
                        `Event "${clusterDef.name}/${eventName}" is REQUIRED by supportedFeatures: ${Diagnosticon(
                            supportedFeatures,
                        )} but is not set!`,
                    );
                }
            ...
            if (!conditionHasMatched && optionalIf !== undefined && optionalIf.length > 0) {
                ...
                if (conditionMatched && (supportedEvents as any)[eventName] === undefined) {
                    logger.debug(
                        `Event "${clusterDef.name}/${eventName}" is optional by supportedFeatures: ${Diagnosticon(
                            supportedFeatures,
                        )} and is not set!`,
                    );
                }
            ...
            if (!conditionHasMatched && (supportedEvents as any)[eventName] !== undefined) {
                logger.warn(
                    `Event "${
                        clusterDef.name
                    }/${eventName}" is provided but it's neither optional or mandatory for supportedFeatures: ${Diagnosticon(
                        supportedFeatures,
                    )} but is set!`,
                );
            }
